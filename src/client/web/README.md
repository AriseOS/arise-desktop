...
# Edit configuration (database, server, security settings)
# vim config/backend.yaml
```
...
**后端配置文件**: `config/backend.yaml`

主要配置项：
- **数据库配置**: 数据库 URL 或文件路径（默认：`dbfiles/agentcrafter.db`）
- **服务器配置**: 主机地址、端口、热重载设置
- **安全配置**: JWT 密钥、算法、过期时间
- **日志配置**: 日志级别

**环境变量覆盖**: 可以通过环境变量覆盖配置文件
- `BACKEND_SERVER_PORT` - 覆盖服务器端口
- `BACKEND_DATABASE_PATH` - 覆盖数据库路径
- `BACKEND_SECURITY_SECRET_KEY` - 覆盖 JWT 密钥
- 等等...

**配置优先级**:
1. 环境变量 > 2. YAML 配置文件 > 3. 代码默认值

### 数据库
- 自动初始化 SQLite 数据库
- 默认位置：`dbfiles/agentcrafter.db`（可在 `config/backend.yaml` 中配置）
