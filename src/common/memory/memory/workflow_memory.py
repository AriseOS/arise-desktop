...
from src.common.memory.memory.memory import (
    ActionManager,
...
class GraphManageManager(ManageManager):
    ...
    def __init__(self, graph_store: GraphStore):
        ...
        """
        self.graph_store = graph_store
        # Use lowercase to match SurrealDB table name
...
class GraphActionManager(ActionManager):
    ...
    def create_action(self, action: Action) -> bool:
        ...
        try:
            ...
            # Use unified "action" as edge type (lowercase to match SurrealDB table)
            # action.type is stored in properties
            rel_type = "action"
    ...
    def get_action(self, source_id: str, target_id: str) -> Optional[Action]:
        ...
        try:
            ...
            # Use unified "action" edge type (lowercase to match SurrealDB table)
            rels = self.graph_store.query_relationships(
                start_node_label=self.node_label,
    ...
    def find_shortest_path(
        ...
        Attempts to use native graph query if available (SurrealDB),
        falls back to BFS if not supported.
        ...
        # Try native graph query first (SurrealDB)
        if hasattr(self.graph_store, 'get_workflow_path'):
            try:
                result = self.graph_store.get_workflow_path(source_id, target_id)
                if result:
                    # Convert to State and Action objects
                    states = []
                    if state_manager:
                        for state_dict in result.get("states", []):
                            state_id = state_dict.get("id") if isinstance(state_dict, dict) else state_dict
                ...
                logger.warning(f"Native graph path query failed, falling back to BFS: {e}")

        # Fallback to BFS
        return self._find_shortest_path_bfs(source_id, target_id, state_manager)

    def _find_shortest_path_bfs(
        self,
        source_id: str,
        target_id: str,
        state_manager: Optional["GraphStateManager"] = None,
    ) -> Optional[Tuple[List["State"], List[Action]]]:
        """Find shortest path using BFS (fallback method).
        ...
        try:
            ...
            logger.error(f" finding shortest path (BFS): {e}")
            return None
...
class GraphCognitivePhraseManager(CognitivePhraseManager):
    ...
    def get_phrase(self, phrase_id: str) -> Optional[CognitivePhrase]:
        ...
        try:
            ...
            if node:
                return CognitivePhrase.from_dict(node)
            return None
        except Exception as e:
        ...
        try:
            ...
            for node in nodes:
                try:
                    ...
                    logger.error(f"Failed to convert node to CognitivePhrase: {e}, node={node}")
                    continue
...
class WorkflowMemory(Memory):
    ...
    # These methods provide complete data structures for API responses,
    # combining data from multiple managers.

    def get_phrase_detail(self, phrase_id: str) -> Optional[Dict[str, Any]]:
        """Get complete CognitivePhrase detail with states and intent sequences.

        This method returns all data needed to display a phrase in the UI,
        including the phrase itself, all referenced states, and all
        intent sequences from the execution plan.
        ...
        Returns:
            Dictionary with:
                - phrase: CognitivePhrase dict
                - states: List of State dicts
                - intent_sequences: List of IntentSequence dicts
            Or None if phrase not found.
        """
        # Get the phrase
        phrase = self.phrase_manager.get_phrase(phrase_id)
        if not phrase:
            return None
        ...
        for state_id in phrase.state_path:
            ...
            if state:
                ...
                # Remove embedding_vector to reduce response size
        ...
        if phrase.execution_plan and self.intent_sequence_manager:
            for step in phrase.execution_plan:
                # Get in-page sequences
                for seq_id in step.in_page_sequence_ids:
                    if seq_id not in seen_sequence_ids:
                        seen_sequence_ids.add(seq_id)
                        seq = self.intent_sequence_manager.get_sequence(seq_id)
                        if seq:
                ...
                if step.navigation_sequence_id and step.navigation_sequence_id not in seen_sequence_ids:
                    seen_sequence_ids.add(step.navigation_sequence_id)
                    seq = self.intent_sequence_manager.get_sequence(step.navigation_sequence_id)
                    if seq:
                        seq_dict = seq.to_dict()
                        seq_dict.pop("embedding_vector", None)
                        intent_sequences.append(seq_dict)
        ...
        phrase_dict.pop("embedding_vector", None)
    ...
    def get_state_with_sequences(self, url: str) -> Optional[Dict[str, Any]]:
        """Get State and its IntentSequences by URL.

        Args:
            url: Page URL to look up.
        ...
        """
        state = self.find_state_by_url(url)
        if not state:
            return None
        ...
        if self.intent_sequence_manager:
            seq_list = self.intent_sequence_manager.list_by_state(state.id)
            for seq in seq_list:
                seq_dict = seq.to_dict()
                seq_dict.pop("embedding_vector", None)
    ...
    def get_memory_stats(self) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with:
                ...
                - total_intent_sequences: Number of intent sequences
                - total_page_instances: Number of page instances
                - total_actions: Number of actions
                - total_phrases: Number of cognitive phrases
        ...
        if self.intent_sequence_manager:
            try:
                all_seq_nodes = self.graph_store.query_nodes(label="IntentSequence")
        ...
        # Get URL index stats
        url_index_stats = self.url_index.get_stats()
    ...
    def clear_all(self) -> Dict[str, int]:
        ...
        has_delete_all = hasattr(self.graph_store, 'delete_all_nodes_by_label')

        if has_delete_all:
            # Use efficient bulk deletion
            states_count = self.graph_store.delete_all_nodes_by_label("State")
            ...
            for domain in all_domains:
                self.domain_manager.delete_domain(domain.id)
            for phrase in all_phrases:
                self.phrase_manager.delete_phrase(phrase.id)

        # Clear URL index
        self.url_index.clear()

        logger.info(
            f"Memory cleared: {states_count} states, {actions_count} actions, "
            f"{domains_count} domains, {phrases_count} phrases, {sequences_count} sequences"
        )
