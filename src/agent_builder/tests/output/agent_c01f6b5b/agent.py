...
class Agent_Generated(BaseAgent):
    ...
    def __init__(self, config: AgentConfig):
        ...
        self.workflow = None
        self.workflow_name = "agentbuilder_workflow_build_c0"
    ...
    async def _setup_workflow(self):
        ...
        builder.add_tool_step(
            name="收集Wiki活动数据",
            instruction="导航到提供的Wiki URL并提取当天的活动数据。",
            tools=['browser_use'],
            description="使用browser_use工具自动导航到用户的Wiki页面，提取当天的活动数据。输入为Wiki URL，输出为活动数据的文本格式。",
            inputs={'wiki_url': '{{user_input}}'},
            outputs={'activity_data': 'step1_output', 'metadata': 'step1_metadata'},
            constraints=['必须是有效的Wiki页面URL', '提取的数据应当是当天的活动'],
        ...
        builder.add_text_step(
            name="生成工作报告",
            instruction="使用llm_extract工具对输入的活动数据进行处理，生成格式化的工作报告。",
            description="利用llm_extract工具对收集到的活动数据进行摘要和重组，生成格式化的工作报告。",
            inputs={'activity_data': '{{step1_output}}'},
            outputs={'report_text': 'step2_output'},
            constraints=['使用llm_extract工具', '确保输出格式化为工作报告'],
        ...
        builder.add_tool_step(
            name="发送报告至微信",
            instruction="使用browser_use工具将报告发送至指定的微信领导。",
            tools=['browser_use'],
            description="通过调用微信API或使用browser_use工具模拟网页操作，将生成的报告发送给指定的领导。输入为报告文本，输出为发送成功的确认。",
            inputs={'report_text': '{{step2_output}}'},
            outputs={'confirmation': 'step3_confirmation'},
            constraints=['确保报告文本格式正确', '确认领导的微信账号已知'],
            confidence_threshold=0.8,
            timeout=60,
            retry_count=3,
        )
