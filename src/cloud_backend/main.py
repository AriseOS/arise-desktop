        ...
        try:
            ...
            if recording_id:
                ...
                if graph:
                    ...
                    yield f"data: {json.dumps({'status': 'analyzing', 'progress': 15, 'message': 'Using State/Action Graph'})}\n\n"
                else:
            ...
            if not graph and not local_intent_sequence:
                ...
                if local_operations:
                    ...
                    yield f"data: {json.dumps({'status': 'understanding', 'progress': 30, 'message': f'Extracted {len(local_intent_sequence)} intents (legacy path)'})}\n\n"

            # Verify we have either graph or intent_sequence
            if not graph and not local_intent_sequence:
                yield f"data: {json.dumps({'status': 'failed', 'progress': 0, 'message': 'No graph, intent sequence, or operations'})}\n\n"
                return
            ...
            request = GenerationRequest(
                ...
                user_query=user_query,
                graph=graph,  # NEW: Graph from Graph Builder
                intent_sequence=local_intent_sequence,  # LEGACY: Intent extraction path
                enable_semantic_validation=enable_semantic_validation
            )
            ...
            async for progress in service.generate_stream(request):
                ...
                logger.info(f"📊 [Stream] Progress: {progress.status.value} - {progress.progress}% - {progress.message}")
                yield f"data: {json.dumps(event_data)}\n\n"
                await asyncio.sleep(0.01)  # Small delay to ensure each event is flushed
            ...
            if response.success:
                ...
                completion_event = json.dumps({'status': 'completed', 'progress': 100, 'workflow_id': workflow_id, 'workflow_name': workflow_name, 'message': 'Workflow generated successfully'})
                logger.info(f"✅ [Stream] Workflow saved successfully, sending FINAL completion event with workflow_id")
                logger.info(f"✅ [Stream] Event payload: {completion_event}")
                yield f"data: {completion_event}\n\n"
                logger.info(f"✅ [Stream] FINAL completion event yielded")

                # Add explicit delay to ensure the final event is fully flushed through all buffers
                # This is critical for SSE streams where the generator closure triggers connection close
                await asyncio.sleep(0.5)
                logger.info(f"✅ [Stream] Delay complete, stream will now close, generator ending")
            else:
