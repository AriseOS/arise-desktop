...
Agent 正在执行 "提取 19 个产品"
  → 提取了产品 1-5
  → 调用 replan_complete_and_handoff(
      summary="产品 1-5: [数据]",
      remaining_tasks=[
        "提取产品 6-10",
        "提取产品 11-15",
...
子任务: "提取 20 款 AI 眼镜"
  → Agent 提取了 5 款
  → Agent 调用 replan_report_progress(5, 20, "已提取 5 款")
  → Agent 调用 replan_complete_and_handoff(
      summary="第 1-5 款: [数据]",
      remaining_tasks=["提取第 6-10 款", "提取第 11-15 款", "提取第 16-20 款"]
    )
  → 创建 3 个新子任务
  → 每个后续子任务接续前一个的进度
  → 最终结果: 4 个子任务共完成 20 款
...
- "撰写第 1-3 节" → complete_and_handoff → "撰写第 4-6 节", "撰写第 7-9 节"
...
- "提取第 6-10 款" → complete_and_handoff → "提取第 6-8 款", "提取第 9-10 款"
...
| 文件 | 操作 | 变更内容 |
|------|------|----------|
| `base_agent/tools/toolkits/replan_toolkit.py` | **新建** | ReplanToolkit 类，包含 4 个工具 |
...
class ReplanToolkit(BaseToolkit):
    """Agent 主动发起的执行时动态任务拆分。"""

    def __init__(self, executor, current_subtask_id: str):
        self._executor = executor          # AMITaskExecutor 实时引用
        self._current_subtask_id = current_subtask_id
        self._handoff_result = None        # 由 complete_and_handoff 设置

    def replan_get_subtask_list(self) -> str:
        """查看所有子任务的 id、类型、状态、内容预览。"""

    def replan_report_progress(self, items_completed: int, items_total: int, details: str) -> str:
        """报告当前进度。发送 AgentReportData SSE 事件。"""

    def replan_add_tasks(self, tasks: str) -> str:
        """追加后续任务。参数为 JSON 字符串：[{"content": "...", "agent_type": "browser"}, ...]
        校验 agent_type 必须有效。调用 executor.add_subtasks()。"""

    def replan_complete_and_handoff(self, summary: str, remaining_tasks: str) -> str:
        """完成当前子任务并交接剩余工作。
        summary 成为当前子任务的 result。
        remaining_tasks 格式同 replan_add_tasks。
        返回 "HANDOFF COMPLETE. Stop now." 通知 Agent 停止。"""
```
...
1. **单元验证**：编写脚本实例化 executor + toolkit，调用 `replan_add_tasks()`，验证子任务列表正确增长
